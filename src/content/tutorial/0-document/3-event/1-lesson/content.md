...
Events are actions that happen in the browser that you can listen to and respond to. For example, when a user clicks a button, that's an event. When a user submits a form, that's an event. When a user presses a key, that's an event. When the browser finishes loading a page, that's an event.
...
:::tip
Your goal is to keep watching all the videos on the LMS and read the JavaScript Note. Don't forget to practice coding every day. You can do it! 🚀
:::

:::info
Remember this code snippet I dropped on Slack for the LMS create-react-app issues on Windows
:::
